...
## 同步和异步
- 同步
	代码执行必须要前一句代码执行完了，才能执行后面的
- 异步
	不需要等待异步代码执行完，就可以执行后面的代码，等异步代码结果出来后，再处理异步代码结果
...
## 闭包
> 闭包属于一种特殊的作用域，称为 静态作用域。它的定义可以理解为: 父函数被销毁 的情况下，子函数的[[scope]]中仍然保留着父级的单变量对象和作用域链，因此可以继续访问到父级的变量对象，这样的函数称为闭包。
...
- Vue在vue.config.js中设置proxy，devServer
- React在package.json中或者在setuoProxy.js中写
